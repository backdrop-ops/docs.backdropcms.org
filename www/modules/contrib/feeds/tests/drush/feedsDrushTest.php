 ...
 * @code
 * UNISH_NO_TIMEOUTS=1 UNISH_BACKDROP_MAJOR_VERSION=1 /path/to/drush/vendor/bin/phpunit --configuration /path/to/drush/tests /path/to/feeds/tests/drush
 * @endcode
...
class feedsDrushTest extends CommandUnishTestCase {
  ...
  public function setUp() {
    if (UNISH_BACKDROP_MAJOR_VERSION != 1) {
      $this->markTestSkipped('This version of Feeds is for Backdrop.');
    }

    // Install the standard install profile.
    $site = $this->setUpBackdrop(1, TRUE, UNISH_BACKDROP_MAJOR_VERSION, 'standard');
    $root = $this->webroot();
  ...
  protected function getFeedsDir() {
    return $this->webroot() . '/' . $this->backdropSitewideDirectory() . '/modules/feeds';
  }
